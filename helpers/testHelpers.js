...
import { Context } from "bpmn-elements";
import { Environment } from "bpmn-elements";
import { Serializer, TypeResolver } from "moddle-context-serializer";
import { Scripts } from "./JavaScripts.js";

const camundaBpmnModdle = JSON.parse(
  fs.readFileSync("./node_modules/camunda-bpmn-moddle/resources/camunda.json")
);
...
function AssertMessage(processContext, messages, inSequence) {
  return function assertMessage(routingKey, activityId, compareState) {
    if (!messages.length) {
      if (activityId)
        throw new Error(`${routingKey} <${activityId}> not found`);
    ...
    if (!inSequence) {
      if (message.fields.routingKey !== routingKey)
        return assertMessage(routingKey, activityId);
      if (activityId && message.content.id !== activityId)
        return assertMessage(routingKey, activityId);
    }

    expect(
      message.fields,
      `${message.fields.routingKey} <${message.content.id}>`
    ).to.have.property("routingKey", routingKey);
    ...
    expect(
      activityApi.getState(),
      `${routingKey} ${activityId} state`
    ).to.deep.include(compareState);
